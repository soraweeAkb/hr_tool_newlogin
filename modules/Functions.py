...
def set_format(wb, file_path, id):
    ws = wb.active
    if ws.title == 'Sheet':
        ws.title = str(id)
    else:
    ...
    ws.row_dimensions[5].height = 0.5
    ...
    for i in range(8, 42):
        ...
        for j in range(2, 22):
            ws.cell(row=i, column=j).alignment=AlignOBJ

        for j in range(4, 16):
            ws.cell(row=i, column=j).number_format="h:mm;@"
        ws.cell(row=i, column=18).number_format="h:mm;@"

        for j in [16, 17, 19]:
            ws.cell(row=i, column=j).alignment=Alignment(horizontal='right', vertical='center', wrap_text=True)

        for j in [16, 17, 19]:
            ws.cell(row=i, column=j).number_format="0.00_ ;[Red]-0.00 "

        for j in [20, 21]:
            ws.cell(row=i, column=j).number_format="0"

    ws['Q42'].value='=COUNT(Q8:Q41)*8'
    ws['Q42'].number_format="#,##0.00_);[Red](#,##0.00)"

    ws['S42'].value='=SUM(S8:S41)'
    ws['S42'].number_format="0.00_ ;[Red]-0.00 "

    ws['T42'].value='=SUM(T8:T41)'
    ws['T42'].number_format="#,##0_ ;[Red]-#,##0 "
    ws['T42'].alignment= Alignment(horizontal='center', vertical='center')

    ws['U42'].value='=SUM(U8:U41)'
    ws['U42'].number_format="#,##0_ ;[Red]-#,##0 "
    ws['U42'].alignment= Alignment(horizontal='center', vertical='center')

    ws['S43'].number_format = "0.00_ ;[Red]-0.00 "
    ws['S44'].number_format = "0.00_ ;[Red]-0.00 "

    ws['H45'].border=Border(bottom=Side(border_style='thin', color='000000'))
    ws['I45'].border=Border(bottom=Side(border_style='thin', color='000000'))
    ws['J45'].border=Border(bottom=Side(border_style='thin', color='000000'))
    ws['K45'].border=Border(bottom=Side(border_style='thin', color='000000'))
    ws['L45'].border=Border(bottom=Side(border_style='thin', color='000000'))
    ws['M45'].border=Border(bottom=Side(border_style='thin', color='000000'))
    ws['N45'].border=Border(bottom=Side(border_style='thin', color='000000'))

    ws['P45'].border=Border(bottom=Side(border_style='thin', color='000000'))
    ws['Q45'].border=Border(bottom=Side(border_style='thin', color='000000'))
    ws['R45'].border=Border(bottom=Side(border_style='thin', color='000000'))
    ws['S45'].border=Border(bottom=Side(border_style='thin', color='000000'))
    ws['T45'].border=Border(bottom=Side(border_style='thin', color='000000'))
    ws['U45'].border=Border(bottom=Side(border_style='thin', color='000000'))

    ws['H45'].alignment=Alignment(horizontal='left', vertical='center', wrap_text=False)
    ws['P45'].alignment = Alignment(horizontal='left', vertical='center', wrap_text=False)
